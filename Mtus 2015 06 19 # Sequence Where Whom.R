...
length(rc)
# 
matWIT_check_seq = seqdef(matWIT_check, cpal = rc) 
...
SoloNight = ifelse(matWIT[,1:20] == 'alone', 1, 0)
sd = seqdef(SoloNight)
...
for(j in 1:nrow(matSolo)){
  for(i in 1:ncol(matSolo)){
    if(famy[i,j] == "work study sleep"){
      matSolo[i,j] == 99
    }
  }
}
...
mat1 = matwit5[1:10,20:40]
mat1.wake = ifelse(mat1 == 1, 0, 1)

mat2 = matWIT[1:10,20:40]
mat2 = ifelse(mat2 == 'partner' | mat2 == 'nuclear', 1, 0)
